    ...
    <div class="page_content">
        ...
        <template v-else>
            ...
            <div class="margin-top-50">
                <div class="box">
                    <div class="textinBox">
                        <h4>{{ $t('company_jobs_page.addnewjobbtn') }}</h4>
                        <!-- <p>Preparing your new jobs throw those filter</p> -->
                    </div>
                    <hr />
                    <form class="form-type-1" @submit.prevent="handleJobEdit">
                                ...
                                <div class="editor job_edit_editor">
                                    ...
                                    <editor-floating-menu :editor="editor">
                                        <div slot-scope="{ commands, isActive, menu }" class="editor__floating-menu" :class="{ 'is-active': menu.isActive }" :style="`top: ${menu.top}px`">
                                            ...
                                            <button class="menubar__button simple_link link_green" @click="insertJobTemplate()" type="button">Insert template by HR bee</button>
                                        </div>
                        ...
                        <div class="container">
                            <div class="row">
                                <div class="col-md-6 m-t-10">
                                    {{ $t('company_jobs_page.requiredreferenceandmotivation') }}
                                </div>
                                <div class="col-md-3">
                                    <div class="form__group">
                                        <b-form-checkbox v-model="requiredReference" @change="toggleReferenceLetter">
                                ...
                                <div class="col-md-3">
                                    <div class="form__group">
                                        <b-form-checkbox v-model="requiredMotivation" @change="toggleMotivationLetter">
                        ...
                        <div class="text-center">
                            <button class="site_btn btn_purple1 m-t-10 m-r-10" type="submit">
                                {{ $t('company_jobs_page.publishjobbtn') }}
                            </button>
                            <button class="site_btn btn_green m-t-10" type="button" v-if="this.html !== '' && this.job_title !== '' && !alredy_jobtemplate && !justSavedAsTemplate" @click="saveAsTemplate">
                                {{ $t('company_jobs_page.saveastemplatebtn') }}
                            </button>
                        </div>
                    </form>
                </div>
        ...
        <!-- <div class="mt-5 mb-5">
            ...
            <p v-if="loadingOpenAi">Loading ...</p>
            <div class="d-flex">
                <div class="form-type-1" style="width: 100%;">
                    <div class="form__group">
                        <input class="form__input" type="text" placeholder="..." v-model="message" name="message" />
                    </div>
                </div>
                <button class="site_btn btn_green" @click="chat()">Send</button>
            </div>
        </div> -->
    </div>
</template>
...
export default {
    ...
    computed: {
        ...
        current_site_language() {
            ...
            this.fillStatusData(this.$i18n.locale())
    ...
    methods: {
        ...
        //         model: 'gpt-3.5-turbo'
        //     })
        //     console.log("return", chatCompletion.choices[0].message);

        //     const resultModel = chatCompletion.choices[0].message;

        //     this.messages.push({ role: 'assistant', content: resultModel.content });
            ...
            this.$validator.validate().then((valid) => {
                if (valid) {
                    let submitted_user_data = {
                        ...
                        job_description: this.removeImgTags(this.html),
                        job_image_base64: this.job_image_base64,
                    }

                    setTimeout(() => {
                        this.$store
                            .dispatch('company_data/recruitment/template_jobs/add_template_job', submitted_user_data)
                            .then((response) => {
                                // Handle the alerts
                                ...
                                if (response.status == 200) {
                                    this.$toast.success('Saved As Template', 'Success', { timeut: 3000 })
                                    this.closeModal()
                                    this.justSavedAsTemplate = true
                                }
        ...
        toggleReferenceLetter: function (checked) {
            if (checked) {
        ...
        toggleMotivationLetter: function (checked) {
            if (checked) {
                this.requiredMotivation = true
            } else {
                this.requiredMotivation = false
            }
        },
        ...
        fillStatusData(current_language) {
            this.statusjob = []
            this.statusjobOptions = []
        ...
        changeJobStatus: function (job_status) {
            if (this.current_job_status == job_status) {
                alert('You job already has this status.')
                return
            }
            switch (job_status) {
                case 'active':
        ...
        handleJobEdit: function () {
            ...
            if (!this.country.name || !this.city.value || !this.city.name || !this.country.value) {
                ...
                this.$validator.validate().then((valid) => {
                    if (valid) {
                        ...
                        let reqReference = 0
                        if (this.requiredReference === false) {
                            reqReference = 0
                        } else {
                            reqReference = 1
                        }
                        ...
                        let reqMotivation = 0
                        if (this.requiredMotivation === false) {
                            reqMotivation = 0
                        } else {
                            reqMotivation = 1
                        }
                        ...
                        let submitted_user_data = {
                            ...
                            required_reference: reqReference,
                            required_motivation: reqMotivation,
                            job_image_base64: this.job_image_base64,
                        }

                        setTimeout(() => {
                            this.$store
                                .dispatch('company_data/recruitment/jobs/add_job', submitted_user_data)
                                .then((response) => {
                                    if (response.status == 200) {
                                        this.$toast.success(this.$t('alertscompany.job_addeed'), 'Success', { timeut: 3000 })
    ...
    created() {
        ...
        let jobTemplate = window.location.search
        if (jobTemplate.includes('?jobTemplate=')) {
            jobTemplate = jobTemplate.split('=')[1]
            this.alredy_jobtemplate = true

            setTimeout(() => {
                this.$store
                    .dispatch('company_data/recruitment/template_jobs/get_single_template_job', jobTemplate)
                    .then((response) => {
                        this.editor.setContent(response.data.job_description)
                        this.html = response.data.job_description
                        this.job_title = response.data.title
                        this.category = {
                            name: this.$t('businessCategories.' + response.data.category_value),
                            value: response.data.category_value,
                        }
                        this.country = {
                            name: response.data.country_name,
                            value: response.data.country_value,
                        }
