    ...
    <div class="page_content">
        ...
        <div v-else>
            <template v-if="created_jobs.total != '0'">
                ...
                <div class="row" v-if="created_jobs && created_jobs.data.length">
                    <div class="col-md-6">
                        <button class="site_btn btn_purple1 m-t-20" @click="downloadReport()">
                            {{ $t('account_settings.downloadreportall') }}
                        </button>
                    </div>
                </div>
                            ...
                            <div class="company-job-page">
                                ...
                                <div class="text-center">
                                    <template v-if="listing_item.total_applications != 0">
                                        <button class="btn slcCandidate" @click="downloadReport(listing_item.job_id)">
                                            {{ $t('account_settings.downloadreport') }}
                                        </button>
                                    </template>
                                    <template v-else>
                                        <button class="btn slcCandidateDisable" :to="'/app/company/company-jobs-archive/' + listing_item.job_id + '/details'">
                                            {{ $t('account_settings.downloadreport') }}
                                        </button>
                                    </template>
                                </div>
                ...
                <br />
                <b-pagination v-if="created_jobs.total > 9" align="center" @change="loadAnotherPage" :total-rows="created_jobs.total" v-model="created_jobs.current_page" :per-page="created_jobs.per_page"> </b-pagination>
            </template>
            <template v-else>
    ...
    methods: {
        ...
        loadAnotherPage: function (page_number_to) {
            ...
            if (this.filteringJobs) {
                ...
                setTimeout(() => {
                    this.$store.dispatch('company_data/recruitment/jobs/get_jobs_with_page', page_number_to).then((response) => {
                        this.component_loading = false
                    })
                }, 200)
            }
        ...
        downloadReport: function (jobId) {
            const reportType = !jobId ? 'recruitment_report_all' : 'recruitment_report_for_job'
            let data = {
                reportType,
            }
            if (jobId) data.jobId = jobId
            this.$store
                .dispatch('company_data/download_report', data)
                .then((response) => {
