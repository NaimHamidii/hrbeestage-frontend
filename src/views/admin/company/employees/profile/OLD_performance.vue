<template>
    <div>
        <div class="page_title has-back-arrow has-menu-right m-t-20">
            <ul class="header_menu_right">
                <li>
                    <a @click="open_show_all_performances_modal" class="site_btn btn_green">
                        {{ $t('employee_module.addnewperformance') }}
                    </a>
                </li>
            </ul>
        </div>
                ...
                <div class="company_performance_widgets">
                    ...
                    <template v-else>
                        <div class="row m-t-20">
                            <div class="col-md-6 m-b-10" v-for="widget_item in employeeAddedPerformances" :key="widget_item.seq_id">
                                <div class="boxi">
                                    <div class="row">
                                        <div class="col-2">
        ...
        <div class="profile_edit_modals performance_modal" v-if="modal_visible">
            ...
            <div class="modal_item" v-if="add_method_modal">
                ...
                <form class="form-type-1" @submit.prevent="addNewPerformance">
                    <div class="textinBox">
                        <h4 class="m-b-30">{{ $t('employee_module.performancetype') }} {{ name }}</h4>
                    </div>

                    <div class="form__group">
                        <label for="interview_category" class="form__label bold">
                            {{ $t('employee_module.rateperformance') }}
                            - {{ rating }}/10
                        </label>
                    ...
                    <div class="form__inline last-input">
                        <div class="form__group">
                            <label for="for_year" class="form__label bold">{{ $t('employee_module.yearforrating') }}</label>
                            <multiselect v-validate="'required'" id="for_year" name="for_year" v-model="for_year" :options="for_yearOptions" :placeholder="$t('employee_module.yearforratingdc')" label="name" track-by="value"></multiselect>
                            <span v-if="submitted && errors.has('for_year')" class="form-error-notification error">
                                {{ $t('inputError.required') }}
                            </span>
                        </div>
                        <div class="form__group">
                            <label for="for_month" class="form__label bold">{{ $t('employee_module.monthforrating') }}</label>
                            <multiselect v-validate="'required'" id="for_month" name="for_month" v-model="for_month" :options="for_monthOptions" :placeholder="$t('employee_module.monthforratingdc')" label="name" track-by="value"></multiselect>
            ...
            <div class="modal_item" v-if="show_all_company_performances_modal">
                <div class="close_modal m-t-10 m-r-10" @click="close_show_all_performances_modal">
                    <img src="/images/close.svg" width="20px" />
                </div>

                <form class="form-type-1">
                    <div class="textinBox">
                        <h4 class="m-b-30">{{ $t('employee_module.selecttypeperformance') }}</h4>
                    </div>

                    <div class="boxi experience_listing modal_performance_listings" :class="{ no_data: all_company_performances.length == 0 }">
                        ...
                        <template v-else>
                            <template v-if="all_company_performances.length == 0">
                                <div class="modal_no_performances_added">
                                    <h5>{{ $t('employee_module.noperformance') }}</h5>
                                    <p>{{ $t('employee_module.noperformancedc') }}</p>
                                </div>
                            </template>
                            <template v-else>
                                <div class="row">
                                    <div class="col-md-4 m-b-40" v-for="criteria_item in all_company_performances" :key="criteria_item.performance_definition_id">
                                        <div class="text-center">
