<template>
    <div class="page_content">
        <div class="page_title has-back-arrow has-menu-right m-b-30">
            <h4>Documents</h4>
            <ul class="header_menu_right" v-if="current_employee_documents.total != 0">
                <li>
                    <a @click="open_modal_organogram_upload" class="site_btn btn_green"> Add new document </a>
                </li>
            </ul>
        </div>
        ...
        <template v-else>
            <div class="documents_container">
                <div class="document_filters">
                    <h5>Categories</h5>
                    <ul>
                        <li @click="filterDocuments('show_all')" :class="{ active: current_active_filter == 'show_all' }">Show all</li>
                        <li v-for="filter_item in document_typeOptions" @click="filterDocuments(filter_item.value)" :key="filter_item.value" :class="{ active: current_active_filter == filter_item.value }">
                            {{ filter_item.name }}
                        </li>
                    </ul>
                </div>
                <div class="document_files">
                    ...
                    <template v-else>
                        <template v-if="current_employee_documents.total > '0'">
                            <div class="document_item" v-for="document_item in current_employee_documents.data">
                                <div class="document_icon" :class="{ image: document_item.original_file_extension == 'jpg' || document_item.original_file_extension == 'png' || document_item.original_file_extension == 'jpeg' }">
                                    ...
                                    <template v-if="document_item.original_file_extension == 'jpg' || document_item.original_file_extension == 'png' || document_item.original_file_extension == 'jpeg'">
                                        <div class="document_image_type" :style="'background-image:url(' + SERVER_URL + IMAGES_FILES + 'employee/' + document_item.saved_file_name + ')'"></div>
                                    </template>

                                    <div class="document_action_delete">
                                        <a target="_blank" :href="SERVER_URL + IMAGES_FILES + 'employee/' + document_item.saved_file_name" class="nav-icon bg--success" v-b-tooltip.hover title="Open document">
                            ...
                            <b-pagination v-if="current_employee_documents.total > '10'" align="left" class="m-t-20" @change="loadAnotherPage" :total-rows="current_employee_documents.total" v-model="current_employee_documents.current_page" :per-page="current_employee_documents.per_page"> </b-pagination>
                        </template>
                        <template v-else>
                            <div class="box cv_status m-b-20">
                                ...
                                <div class="status_text">
                                    <h5>You don't have any documents added.</h5>
                                    <p>Please go ahead and add a new employee document.</p>
                                </div>
        ...
        <div class="profile_edit_modals" v-if="modal_visible">
            ...
            <div class="modal_item edit_profile_image" v-if="modal_organogram_upload">
                ...
                <form class="form-type-1" @submit.prevent="handleOrganogramUpload">
                    <h5 class="add">Add new document</h5>
                    <!-- Document type -->
                    <div class="form__group">
                        <label for="document_type" class="form__label bold">{{ $t('inputs.document_type') }} <span class="required">(*)</span></label>
                        <multiselect id="document_type" v-validate="'required'" name="document_type" v-model="document_type" :options="document_typeOptions" :placeholder="$t('inputs.document_type')" label="name" track-by="value"></multiselect>
                        <span v-show="errors.has('document_type')" class="form-error-notification error">
...
export default {
    ...
    methods: {
        ...
        loadAnotherPage: function (page_number_to) {
            ...
            setTimeout(() => {
                this.$store.dispatch('company_data/employee/documents/get_employee_documents_with_page', document_data).then((response) => {
                    this.documents_loading = false
                })
            }, 400)
        },
