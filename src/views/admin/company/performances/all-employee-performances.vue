<template>
    <div class="page_content">
        ...
        <div class="boxi">
            <div class="textinBox">
                <h4>{{ $t('performance.performance_page_title') }}</h4>
                <p>{{ $t('performance.performance_page_description') }}</p>
                <hr />
            </div>
        </div>
        <div v-if="loading">
            ...
            <div v-if="employees.data.length > 0">
                <div class="row m-t-10">
                    <div class="col-md-4 m-t-10" v-for="employee_item in employees.data" :key="employees.employee_id">
                        <router-link :to="'/app/company/performances/employee/' + employee_item.employee_id">
...
<script>
    ...
    export default {
        ...
        computed: {
            employees: function () {
                if (this.$store.getters['company_data/company_performance_templates/get_employees']) {
                    return this.$store.getters['company_data/company_performance_templates/get_employees']
                }
            },
        },
        watch: {
        ...
        methods: {
            fetchEmployees(page) {
                this.loading = true
                this.$store.dispatch('company_data/company_performance_templates/fetch_employees', page).then((response) => {
                    this.loading = false
                })
            },
            ...
            searchEmployee: function (query) {
                ...
                setTimeout(() => {
                    this.$store.dispatch('company_data/company_performance_templates/fetch_employees_search', data).then((response) => {
                        this.loading = false
                    })
                }, 200)
            },
