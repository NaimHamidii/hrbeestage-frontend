    ...
    <div class="tab_item">
        <div class="box computer_skills_listing">
            <template v-if="other_skills_data.length == 0" class="">
                <div class="no_other_skills_data">
                    <h5>{{ $t('jobseeker_profile_page.no_skill_added') }}</h5>
                    <a @click="open_add_modal" class="simple_link link_purple">{{ $t('jobseeker_profile_page.add_new_skill') }}</a>
                </div>
            </template>

            <template v-else>
                <div class="other_skill_item" v-for="other_skill_item in other_skills_data" :key="other_skill_item.seq_id">
                    <!-- Here we should pass a  dynamic value other_skill_value that gets translated depending on language -->
                    <h4>{{ $t('other_skills.' + other_skill_item.skill_value + '') }}</h4>
                    <div class="other_skill_item_action">
                        <ul>
                            <li>
                                <a @click="deleteOtherSkill(other_skill_item.seq_id)" class="nav-icon bg--error" v-b-tooltip.hover title="Delete skill">
        ...
        <div class="profile_edit_modals" v-if="modal_visible">
            ...
            <div class="modal_item" v-if="add_method_modal">
                ...
                <form class="form-type-1" @submit.prevent="addNewOtherSkill">
                    <h5 class="add m-b-30">{{ $t('jobseeker_profile_page.add_new_skill') }}</h5>
                    <div class="form__group last-input">
                        <label for="availableOtherSkills" class="form__label bold">{{ $t('inputs.select_skill') }}<span class="required">(*)</span></label>
...
export default {
    ...
    methods: {
        ...
        addNewOtherSkill: function () {
            ...
            for (var i = 0, len = this.other_skills_data.length; i < len; i++) {
                if (this.other_skills_data[i].other_skill_value === this.availableOtherSkills.value) {
