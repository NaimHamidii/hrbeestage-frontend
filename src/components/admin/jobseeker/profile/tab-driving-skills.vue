<template>
    <div class="tab_item">
        <div class="box driving_skills_listing">
            <template v-if="driving_skills_data.length == 0" class="">
                <div class="no_driving_skills_data">
                    <h5>{{ $t('jobseeker_profile_page.no_driving_skill_added') }}</h5>
                    <a @click="open_add_modal" class="simple_link link_purple">{{ $t('jobseeker_profile_page.add_new_driving_skill') }}</a>
                </div>
            </template>

            <template v-else>
                <div class="driving_skill_item" v-for="driving_skill_item in driving_skills_data" :key="driving_skill_item.seq_id">
                    <!-- Here we should pass a  dynamic value other_skill_value that gets translated depending on language -->
                    <h4>{{ driving_skill_item.driving_category | uppercase }}</h4>
                    <div class="driving_skill_item_action">
                        <ul>
                            <li>
                                <a @click="deleteDrivingSkill(driving_skill_item.seq_id)" class="nav-icon bg--error" v-b-tooltip.hover title="Delete skill">
                ...
                <div class="add_new_listing">
                    <a @click="open_add_modal">
                        ...
                        {{ $t('jobseeker_profile_page.add_new_driving_skill') }}
                    </a>
                </div>
            </template>
        </div>
        ...
        <div class="profile_edit_modals" v-if="modal_visible">
            ...
            <div class="modal_item" v-if="add_method_modal">
                ...
                <form class="form-type-1" @submit.prevent="addNewDrivingSkill">
                    <h5 class="add m-b-30">{{ $t('jobseeker_profile_page.add_new_driving_skill') }}</h5>
                    <div class="form__group last-input">
                        ...
                        <multiselect id="availableDrivingSkills" v-validate="'required'" name="availableDrivingSkills" v-model="availableDrivingSkills" :options="availableDrivingSkillOptions" :placeholder="$t('inputs.select_skill')" label="name" track-by="value"></multiselect>
...
export default {
    ...
    methods: {
        ...
        addNewDrivingSkill: function () {
            ...
            for (var i = 0, len = this.driving_skills_data.length; i < len; i++) {
                if (this.driving_skills_data[i].other_skill_value === this.availableDrivingSkills.value) {
...
.driving_skills_listing {
    position: relative;
    cursor: initial;
    .no_driving_skills_data {
        padding: 40px;
        h5 {
            color: initial;
        }
